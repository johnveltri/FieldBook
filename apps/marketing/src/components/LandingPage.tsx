  ...
  return <div className={styles.page}>
    ...
    <main id="top">
      ...
      <section id="how-it-works" className={styles.how}><Blueprint /><div className={styles.content}><motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: .3 }} variants={reveal}><h2>How it works</h2></motion.div><motion.div className={styles.steps} initial="hidden" whileInView="visible" viewport={{ once: true, amount: .1 }} variants={stagger}>
        <ProductCard step={1} title="Mobile-optimized tracking" image="/images/live-session.png" imageAlt="FieldSolo live job session screen">Start a live session while you work or log the job later when the day slows down. Add the basics first. Fill in the rest when you have time.</ProductCard>
        <ProductCard step={2} title="Capture what matters" image="/images/job-notes-materials.png" imageAlt="FieldSolo job notes and materials screen">Capture time, materials, notes, revenue, and payment status without the complicated paperwork.</ProductCard>
        <ProductCard step={3} title="Close out loose ends" image="/images/open-jobs.png" imageAlt="FieldSolo open jobs screen">Stay on top of unpaid jobs and incomplete records before they’re forgotten.</ProductCard>
        <ProductCard step={4} title="Understand your margins" image="/images/earnings-summary.png" imageAlt="FieldSolo earnings summary screen">Review revenue, material costs, net profit, and net-per-hour for one job, or the whole month.</ProductCard>
        <ProductCard step={5} title="Learn from your history" image="/images/earnings-profit-ranking.png" imageAlt="FieldSolo earnings profit ranking screen">Search past jobs, see your best and worst jobs, and price smarter next time.</ProductCard>
    ...
    <footer className={styles.footer}><div><strong>FieldSolo<sup>TM</sup></strong><p>Free job tracking for tradespeople who work for themselves.</p></div><nav aria-label="Footer navigation">{navItems.map(([label, id]) => <NavLink key={id} id={id}>{label}</NavLink>)}<NavLink id="waitlist">Waitlist</NavLink><Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link><Link href="/delete-account">Delete account</Link></nav><small>FieldSolo™ is operated by Veltri Ventures LLC. © {new Date().getFullYear()} FieldSolo™. All rights reserved.</small></footer>
  </div>;
}
