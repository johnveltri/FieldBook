...
const navItems = [
  ["How it works", "how-it-works"], ["Pricing", "pricing"], ["FAQ", "faq"],
] as const;
...
export function LandingPage() {
  ...
  return <div className={styles.page}>
    <header className={styles.header}>
      <div className={styles.navShell}><a className={styles.wordmark} href="#top" onClick={(e) => { e.preventDefault(); setMenuOpen(false); window.scrollTo({ top: 0, behavior: "smooth" }); }}>FieldSoli<sup>TM</sup> <em>Beta</em></a>
    ...
    <main id="top">
      ...
      <section className={styles.problem}><motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: .3 }} variants={stagger}><motion.h2 variants={reveal}>Busy does not mean profitable.</motion.h2><motion.div variants={reveal}><p className={styles.sectionLead}>FieldSoli helps you work smarter, not harder:</p><ul>{["Know your net", "Track expenses", "Say no to unprofitable jobs", "Price smarter", "Grow your margins", "Get paid faster"].map((item) => <li key={item}><CheckCircle2 />{item}</li>)}</ul></motion.div></motion.div><motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: .3 }} transition={{ duration: .8, ease: [0.22, 1, .36, 1] }} className={styles.problemImage}><motion.span aria-hidden className={styles.problemAccentRust} animate={{ rotate: [3, 5, 3] }} transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }} /><motion.span aria-hidden className={styles.problemAccentPaper} animate={{ rotate: [-2, -4, -2] }} transition={{ duration: 7, repeat: Infinity, ease: "easeInOut" }} /><motion.div whileHover={{ scale: 1.02, rotate: -1 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} className={styles.problemPhoto}><img src="/images/source-0.png" alt="Tradesperson working on an electrical installation" /></motion.div></motion.div></section>

      <section className={styles.promise}><Blueprint /><motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: .4 }} variants={stagger}><motion.h2 variants={reveal}>One place to understand every job</motion.h2><motion.p variants={reveal}>Your work comes from everywhere.</motion.p><motion.small variants={reveal}>Referrals. Repeat customers. Angi. Thumbtack. Facebook groups. Property managers. Contractors. Calls. Texts. Another job app.</motion.small><motion.blockquote variants={reveal}>FieldSoli brings the job economics into one clear view, no matter where the work came from.</motion.blockquote></motion.div></section>
      ...
      <section id="how-it-works" className={styles.how}><Blueprint /><div className={styles.content}><motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: .3 }} variants={reveal}><h2>How it works</h2></motion.div><motion.div className={styles.steps} initial="hidden" whileInView="visible" viewport={{ once: true, amount: .1 }} variants={stagger}>
        <ProductCard step={1} title="Mobile-optimized tracking" image="/images/live-session.png" imageAlt="FieldSoli live job session screen">Start a live session while you work or log the job later when the day slows down. Add the basics first. Fill in the rest when you have time.</ProductCard>
        <ProductCard step={2} title="Capture what matters" image="/images/job-notes-materials.png" imageAlt="FieldSoli job notes and materials screen">Capture time, materials, notes, revenue, and payment status without the complicated paperwork.</ProductCard>
        <ProductCard step={3} title="Close out loose ends" image="/images/open-jobs.png" imageAlt="FieldSoli open jobs screen">Stay on top of unpaid jobs and incomplete records before they’re forgotten.</ProductCard>
        <ProductCard step={4} title="Understand your margins" image="/images/earnings-summary.png" imageAlt="FieldSoli earnings summary screen">Review revenue, material costs, net profit, and net-per-hour for one job, or the whole month.</ProductCard>
        <ProductCard step={5} title="Learn from your history" image="/images/earnings-profit-ranking.png" imageAlt="FieldSoli earnings profit ranking screen">Search past jobs, see your best and worst jobs, and price smarter next time.</ProductCard>
      </motion.div></div></section>

      <section id="pricing" className={styles.pricing}><Blueprint /><div className={styles.content}><motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: .3 }} variants={reveal}><h2>Free. Seriously.</h2><p className={styles.pricingLead}>Unlimited job and profit tracking is free during beta. If paid plans come later, we’ll explain what changes before anything does.</p></motion.div><div className={styles.pricingGrid}><motion.article initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: .8 }}><h3>Included in free</h3><ul>{["Job tracking", "Time tracking", "Materials tracking", "Job notes", "Job & payment status", "Searchable job history", "Reminders & action items", "Simple profit reporting"].map((item) => <li key={item}><Check />{item}</li>)}</ul></motion.article><motion.article className={styles.futureCard} initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: .8 }}><h3>How do we make money?</h3><p>We don’t believe in charging for the simple stuff. We’re focusing first on building a great free platform for solo operators.</p><p>Expect additional paid tiers in the future to support larger teams, automations, or features designed to run more parts of your business.</p></motion.article></div></div></section>
    ...
    <footer className={styles.footer}><div><strong>FieldSoli<sup>TM</sup></strong><p>Free job tracking for tradespeople who work for themselves.</p></div><nav aria-label="Footer navigation">{navItems.map(([label, id]) => <NavLink key={id} id={id}>{label}</NavLink>)}<NavLink id="waitlist">Waitlist</NavLink><Link href="/support">Support</Link><Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link><Link href="/delete-account">Delete account</Link></nav><small>FieldSoli™ is operated by Veltri Ventures LLC. © {new Date().getFullYear()} FieldSoli. All rights reserved.</small></footer>
  </div>;
}
...
function Waitlist({ submitted, setSubmitted, trades, setTrades, trackingTools, setTrackingTools, jobSources, setJobSources, onSubmit, submitting, errorMessage, validationAttempted }: {
  ...
  return <section id="waitlist" className={styles.waitlist}><Blueprint variant="dots" /><div className={styles.waitlistGrid}><motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: .2 }} variants={stagger}><motion.h2 variants={reveal}>Join the FieldSoli waitlist</motion.h2><motion.p variants={reveal}>We’re inviting a small group of independent tradespeople to try the beta and tell us what&apos;s working, what&apos;s not, and what you&apos;d like to see next.</motion.p></motion.div>
    <motion.form className={styles.form} method="post" action="/api/waitlist" onSubmit={onSubmit} initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: .2 }} transition={{ duration: .8, ease: [0.22, 1, .36, 1], delay: .2 }} whileHover={{ boxShadow: "0 25px 50px -12px rgba(43, 52, 65, 0.15)" }}>
      ...
      <div className={styles.consent}><input id="waitlist-privacy-consent" name="privacyAccepted" value="true" type="checkbox" required /><label htmlFor="waitlist-privacy-consent">I agree to the <Link href="/privacy">Privacy Policy</Link> and <Link href="/terms">Terms</Link> and consent to receive FieldSoli early-access and product updates.</label></div>
