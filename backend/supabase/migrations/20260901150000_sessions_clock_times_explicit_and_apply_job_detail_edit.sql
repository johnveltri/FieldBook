-- Job Detail Edit: duration-only session flag + transactional apply RPC.

alter table public.sessions
  add column if not exists clock_times_explicit boolean not null default true;

comment on column public.sessions.clock_times_explicit is
  'True when the user explicitly set start/end clock times. False for duration-only Edit saves with synthesized storage timestamps.';

-- Extend column grants for client-generated UUID upserts and the new flag.
revoke insert on public.sessions from authenticated;
grant insert (
  id, job_id, user_id, entry_mode, session_status,
  started_at, ended_at, deleted_at, started_tz, clock_times_explicit
) on public.sessions to authenticated;

revoke update on public.sessions from authenticated;
grant update (
  job_id, session_status, started_at, ended_at, deleted_at, started_tz, clock_times_explicit
) on public.sessions to authenticated;

revoke insert on public.notes from authenticated;
grant insert (id, user_id, job_id, session_id, body) on public.notes to authenticated;
...
begin
  ...
  if v_job_patch is null then
    return jsonb_build_object('status', 'error', 'code', 'invalid');
  end if;
  ...
  loop
    if exists (
      ...
      return jsonb_build_object('status', 'error', 'code', 'conflict');
    end if;
