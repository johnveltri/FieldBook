-- Persist partial session clock explicitness (start-only, end-only, or both).

alter table public.sessions
  add column if not exists clock_start_explicit boolean not null default false,
  add column if not exists clock_end_explicit boolean not null default false;

comment on column public.sessions.clock_start_explicit is
  'True when the user explicitly set the session start clock on Edit.';
comment on column public.sessions.clock_end_explicit is
  'True when the user explicitly set the session end clock on Edit.';

update public.sessions
set
  clock_start_explicit = clock_times_explicit,
  clock_end_explicit = clock_times_explicit
where clock_times_explicit = true;
...
grant insert (
  ...
  clock_times_explicit, clock_start_explicit, clock_end_explicit
...
begin
  ...
  loop
    ...
    insert into public.sessions (
      ...
      v_clock_explicit, v_clock_start_explicit, v_clock_end_explicit
    );
  end loop;
