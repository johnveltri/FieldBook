-- Migration test for 20260422120000_job_costs_soft_delete.sql.
--
-- Verifies that the soft-delete migration landed correctly:
--   1. `public.job_costs.deleted_at` column exists and is nullable `timestamptz`.
...
begin
  ...
  if not found then
    raise exception 'job_costs.deleted_at column is missing';
  end if;
  if col.data_type <> 'timestamp with time zone' then
    raise exception 'job_costs.deleted_at should be timestamptz, got %', col.data_type;
  end if;
  if col.is_nullable <> 'YES' then
    raise exception 'job_costs.deleted_at should be nullable';
  end if;
end
$$;
...
begin
  select indexdef
   ...
   where schemaname = 'public'
     and tablename = 'job_costs'
     and indexname = 'job_costs_job_active_idx';

  if not found then
    raise exception 'job_costs_job_active_idx is missing';
  end if;
  if ix.indexdef not like '%deleted_at IS NULL%' then
    raise exception
      'job_costs_job_active_idx must filter deleted_at IS NULL, got: %',
      ix.indexdef;
  end if;
end
$$;
...
begin
  select indexdef
   ...
   where schemaname = 'public'
     and tablename = 'job_costs'
     and indexname = 'job_costs_session_active_idx';

  if not found then
    raise exception 'job_costs_session_active_idx is missing';
  end if;
  if ix.indexdef not like '%deleted_at IS NULL%' then
    raise exception
      'job_costs_session_active_idx must filter deleted_at IS NULL, got: %',
      ix.indexdef;
  end if;
end
$$;
...
--    Seed a throwaway user + job + two job_costs (one soft-deleted) and
...
begin
  ...
  if v_active_count <> 1 then
    raise exception
      'Expected 1 active cost after soft-delete, got %', v_active_count;
  end if;
...
select 'job_costs_soft_delete.test.sql PASSED' as result;
